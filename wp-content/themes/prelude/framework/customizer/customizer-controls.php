...
class Prelude_Customizer_Hr_Control extends WP_Customize_Control {
	public $type = 'hr';
...
class Prelude_Customizer_Heading_Control extends WP_Customize_Control {
	public $type = 'prelude-heading';
	public function render_content() {
		echo '<span class="prelude-customizer-heading">' . esc_html( $this->label ) . '</span>';
	}
}

// Textarea control for the customizer
class Prelude_Customizer_Textarea_Control extends WP_Customize_Control {
	public $type = 'prelude_textarea';
	public $rows = '3';
	public function render_content() { ?>
		<label>
...
class Prelude_Customize_Multicheck_Control extends WP_Customize_Control {
	public $description = '';
	public $subtitle = '';
...
// Sorter Control
class Prelude_Customize_Control_Sorter extends WP_Customize_Control {
	...
	public function render_content() { ?>
		<div class="prelude-sortable">
			...
			echo '<ul id="'. $this->id .'_sortable">'; ?>
				...
				foreach ( $values as $val ) :
					...
					if ( $label ) : ?>
						<li data-value="<?php echo esc_attr( $val ); ?>" class="prelude-sortable-li">
							<?php echo esc_html( $label ); ?>
							<span class="prelude-hide-sortee fa fa-toggle-on"></span>
						</li>
					<?php
					// End if label check
					endif;
				...
				foreach ( $choices as $val => $label ) { ?>
					<li data-value="<?php echo esc_attr( $val ); ?>" class="prelude-sortable-li prelude-hide">
						<?php echo esc_html( $label ); ?>
						<span class="prelude-hide-sortee fa fa-toggle-on fa-rotate-180"></span>
					</li>
				<?php } ?>
			</ul>
		</div><!-- .prelude-sortable -->
		...
		jQuery(document).ready( function($) {
			...
			sortableUl.on( "sortstop", function( event, ui ) {
				preludeUpdateSortableVal();
			} );

			// Toggle classes
			sortableUl.find( 'li' ).each( function() {
				$( this ).find( '.prelude-hide-sortee' ).on('click', function() {
					$( this ).toggleClass( 'fa-rotate-180' ).parents( 'li:eq(0)' ).toggleClass( 'prelude-hide' );
				} );
			})
			// Update Sortable when hidding/showing items
			$( '#<?php echo esc_html( $this->id ); ?>_sortable span.prelude-hide-sortee' ).on('click', function() {
				preludeUpdateSortableVal();
			} );
			// Used to update the sortable input value
			function preludeUpdateSortableVal() {
				var values = [];
				sortableUl.find( 'li' ).each( function() {
					if ( ! $( this ).hasClass( 'prelude-hide' ) ) {
...
class Prelude_Fonts_Dropdown_Custom_Control extends WP_Customize_Control {
	public function render_content() {
	$this_val = $this->value(); ?>
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			<select <?php $this->link(); ?> style="width:100%;">
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default', 'prelude' ); ?></option>
			<?php
			// Add custom fonts from child themes
			if ( function_exists( 'prelude_add_custom_fonts' ) ) {
				$fonts = prelude_add_custom_fonts();
				if ( $fonts && is_array( $fonts ) ) { ?>
					<optgroup label="<?php esc_html_e( 'Custom Fonts', 'prelude' ); ?>">
			...
			$std_fonts = '';
			if ( $std_fonts = prelude_standard_fonts() ) { ?>
				<optgroup label="<?php esc_html_e( 'Standard Fonts', 'prelude' ); ?>">
					<?php
			...
			// Google font options
			$google_fonts = '';
			if ( $google_fonts = prelude_google_fonts_array() ) { ?>
				<optgroup label="<?php esc_html_e( 'Google Fonts', 'prelude' ); ?>">
					<?php
					// Loop through font options and add to select
