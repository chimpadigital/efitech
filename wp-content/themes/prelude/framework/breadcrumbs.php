...
function prelude_breadcrumbs() {

    global $post;

    $home_text     = esc_html__('Home','prelude');
    $category_text = esc_html__('Archive by Category "%s"','prelude');
    $tax_text      = esc_html__('Archive by "%s"','prelude');
    $tag_text      = esc_html__('Posts Tagged "%s"','prelude');
    $author_text   = esc_html__('Articles Posted by %s','prelude');
    $error_text    = esc_html__('Error 404','prelude');
    $search_text   = esc_html__('Search Results for "%s" Query','prelude');

    $blog_text   = esc_html__('Blog','prelude');
	$project_text = esc_html__('Projects','prelude');
	$product_text = esc_html__('Shop','prelude');
                    ...
                    -------------------*/
                        if ( $cpt->name == 'project' ) {
                            if ( is_singular( 'project' ) ) {
                                ...
                                $page_id = prelude_parse_obj_id( prelude_get_mod( 'portfolio_page' ), 'page' );
