...
function rgmk_add_admin_menu_html() {
	...
	if ( $updated ) {
		echo '<div class="updated fade"><p><strong>' . __( 'Kay Updated!', 'prelude' ) . '</strong></p></div>';

	}
	?>
	<div class="wrap">
		<?php echo get_option( 'rgmk_google_map_api_key' ); ?>
		<h2><?php _e( 'Google Maps API KEY', 'prelude' ); ?></h2>
		<p>
			...
			<a href='https://console.developers.google.com/henhouse/?pb=["hh-1","maps_backend",null,[],"https://developers.google.com",null,["maps_backend","geocoding_backend","directions_backend","distance_matrix_backend","elevation_backend","places_backend"],null]&TB_iframe=true&width=600&height=400'
			   ...
			   name="<?php _e( 'Generate API Key - ( MUST be logged in to your Google account )', 'prelude' ); ?>">
				<?php _e( 'Generate API Key', 'prelude' ); ?>
			</a>
			<?php echo sprintf( __( 'or %sclick here%s to Get a Google Maps API KEY', 'prelude' ), '<a target="_blank" href=\'https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend&keyType=CLIENT_SIDE&reusekey=true\'>', '</a>' ) ?>
		</p>

		<form method="post" action="options-general.php?page=gmaps-api-key">
			<label for="rgmk_google_map_api_key"><?php _e( 'Enter Google Maps API KEY', 'prelude' ); ?></label>
			<input title="<?php _e( 'Add Google Maps API KEY', 'prelude' ); ?>" type="text"
			       name="rgmk_google_map_api_key" id="rgmk_google_map_api_key"
			       placeholder="<?php _e( 'Enter your API KEY here', 'prelude' ); ?>"
