...
if ( ! empty( $images ) ) {
    wp_enqueue_script( 'prelude-owlcarousel' );
    $images = explode( ',', trim( $images ) );

    $html  .= '<div class="prelude-images-carousel '. $cls .'" data-auto="'. $auto_scroll .'" data-loop="'. $loop .'" data-gap="'. $gap .'" data-column="'. $column .'" data-column2="'. $column2 .'" data-column3="'. $column3 .'">
    ...
    for ( $i=0; $i<count( $images ); $i++ ) {
        ...
        $html .= sprintf(
            '<div class="item-wrap"><img src="%1$s" alt="Image"></div>',
            $img_b[0],
            $img_f[0]
        );
    }
