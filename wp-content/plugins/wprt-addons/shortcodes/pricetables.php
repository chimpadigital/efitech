	...
	$prices_html .= '<div class="pricing-item">';
		...
		if ( $data['button_content'] ) {
			...
			if ( $data['button_style'] == 'background' ) {
				if ( $data['button_background'] == '#00d664' ) {
					$btn_cls .= ' accent';
				} else {
					$btn_cls .= ' custom';
		    ...
		    $prices_html .= '<div class="button-wrap"><a target="'. $data['new_tab'] .'" class="prelude-button '. $btn_cls .'" href="'. $data['button_url'] .'" style="'. $btn_css .'" '. $btn_data .'>'. $data['button_content'] .'</a></div><div class="btn-ex">'. $data['button_content_ex'] .'</div>';
		}
...
echo '<div class="prelude-pricing-group">'. $heading_html . $prices_html .'</div>';
