...
if ( $heading ) {
	if ( $heading_color == '#00d664' ) { $h_cls .= ' accent'; }
...
if ( isset( $price ) ) {
	if ( $price_color == '#00d664' ) { $fig_cls .= ' accent'; }
	else { if ( $price_color ) $fig_css .= 'color:'. $price_color .';'; }

	if ( $unit_color == '#00d664' ) { $unit_cls .= ' accent'; }
	else { if ( $unit_color ) $unit_css .= 'color:'. $unit_color .';'; }

	if ( $currency_color == '#00d664' ) { $cur_cls .= ' accent'; }
	...
	if ( $price_font_family != 'Default' ) {
		prelude_enqueue_google_font( $price_font_family );
	...
	if ( $unit_font_family != 'Default' ) {
		prelude_enqueue_google_font( $unit_font_family );
...
if ( $link_text && $link_url ) {
	...
	$button_html .= sprintf(
		'<div class="price-button" style="%7$s">
			<a target="%5$s" class="prelude-button %3$s" href="%2$s" style="%4$s" %6$s>%1$s</a>
		</div>',
		$link_text,
		$link_url,
		$button_cls,
...
printf(
	'<div class="prelude-price-table %1$s" style="%2$s">
		%4$s %5$s
