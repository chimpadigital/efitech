...
if ( $video_url ) {
	...
	$icon_html = sprintf(
		'<div class="prelude-video-icon clearfix %2$s" style="%3$s"><a class="icon-wrap popup-video" href="%1$s"><span class="circle"></span></a></div>',
		$video_url,
		$icon_cls,
		$icon_css
	);
...
if ( $image ) {
	...
	if ( $effect == 'simple' ) {
		...
		printf(
			'<div class="prelude-simple-image simple %3$s" style="%4$s">
				%1$s %2$s
			</div>',
			$image_html,
			$icon_html,
	...
	if ( $effect == 'reveal' ) {
		...
		printf(
			'<div class="prelude-simple-image %3$s %7$s" %5$s %6$s>
	...
	if ( $effect == 'background' ) {
		...
		printf(
			'<div class="prelude-fancy-img %3$s" %5$s %6$s>
				<div class="prelude-fancy-img-inner">
					<span class="prelude-fancy-img-bg"></span>

					<div class="prelude-fancy-img-holder" style="%4$s">
						%1$s %2$s
					</div>
				</div>
			</div>',
