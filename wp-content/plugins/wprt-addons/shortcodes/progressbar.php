...
if ( $line_one == '#00d664' ) { $cls1 .= ' accent';
} else { if ( $line_one ) $css1 = 'background-color:'. $line_one .';'; }

if ( $gradient && function_exists('prelude_hex2rgba') ) {
	$cls1 .= ' gradient';
	$css1 = 'background: '. prelude_hex2rgba($line_one, 1) .';background: -moz-linear-gradient(left, '. prelude_hex2rgba($line_one, 1) .' 0%, '. prelude_hex2rgba($line_one, 0.3) .' 100%);background: -webkit-linear-gradient( left, '. prelude_hex2rgba($line_one, 1) .' 0%, '. prelude_hex2rgba($line_one, 0.3) .' 100% );background: linear-gradient(to right, '. prelude_hex2rgba($line_one, 0.3) .' 0%, '. prelude_hex2rgba($line_one, 1) .' 100%);';
}
...
if ( $font_family != 'Default' ) {
	prelude_enqueue_google_font( $font_family );
	$css .= 'font-family:'. $font_family .';';
}

if ( $per_color == '#00d664' ) { $pcls .= ' accent';
...
wp_enqueue_script( 'prelude-appear' );
printf( '
	<div class="prelude-progress clearfix %7$s" style="%6$s">%1$s %2$s
