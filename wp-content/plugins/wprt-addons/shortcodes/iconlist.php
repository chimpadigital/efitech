...
foreach ( $boxicon as $data ) {
	...
	if ( ! empty( $data[$icon] ) ) {
		...
		$html .= '<div class="prelude-icon '. $icon_cls .'"'. $icon_data .'><span class="icon" style="'. $icon_css .'"><i class="'. $data[$icon] .'"></i></span></div>';
	}
...
echo '<div class="prelude-icon-list clearfix '. $cls .'">'. $html .'</div>';
?>