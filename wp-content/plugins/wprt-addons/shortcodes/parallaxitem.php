...
if ( $image_rounded ) $css .= 'border-radius:'. intval( $image_rounded ) .'px;';

if ( $horizontal && $vertical && $blur && $spread && $shadow_color )
    $css .= 'box-shadow:'. $inset .' '. $horizontal .' '. $vertical .' '. $blur .' '. $spread .' '. $shadow_color .';';

printf(
	'<div class="prelude-parallax-item %7$s" style="%2$s" data-parallax=\'{"y" : %4$s, "smoothness" : %5$s}\' data-top="%6$s">%1$s</div>',
	$image_html,
	$css,
	intval( $parallax_x ),
	intval( $parallax_y ),
